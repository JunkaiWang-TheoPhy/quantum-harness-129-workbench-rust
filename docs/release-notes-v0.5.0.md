...
> **Post-release audit correction:** the v0.5.0 tag is immutable. The
> corrective integration branch reports the C₂ᵥ/A1 energy as
> `-76.24321859 E_h`, labels MaxRSS as transcribed because raw `sacct` is
> unavailable, and discloses that the exact production `src/direct_fci.rs`
> was not archived. These limits do not change the residual-based numerical
> acceptance, but they preclude a byte-for-byte production-source claim.
...
machine-readable result, and submitted batch script are committed. The exact
production `src/direct_fci.rs` and raw Slurm accounting row are unavailable.
PySCF 2.14.0 separately reproduces the same RHF input and provides MP2, CISD,
CCSD, and CCSD(T) hierarchy/scale checks rather than an independent FCI
oracle. The same-input CCSD(T) energy is
...
cargo test --locked --test final_competition_evidence
```
