@chenpeizhi Thank you for encouraging us to test the implementation on HPC
and to separate credible exact-FCI claims from bounded or selected-space
evidence. We have now completed that follow-up and published
[`v0.5.0`](https://github.com/JunkaiWang-TheoPhy/quantum-harness-129-workbench-rust/releases/tag/v0.5.0).

> **Post-release audit correction:** the immutable v0.5.0 tag is retained,
> but the corrective branch reports the large-run energy at precision
> supported by its residual and discloses that the raw `sacct` row and exact
> production `src/direct_fci.rs` are unavailable.

## Continuity from the submitted result

The original Quantum Harness #129 acceptance remains unchanged:

* H2O/6-31G frozen core has 245,025 determinants.
* Direct Davidson FCI remains `-76.121174204141980 E_h`.
* All 8 CC entries and all 28 CI and MBPT entries from Hirata and Bartlett
  Table 2 remain accepted at the six decimal places printed by the paper.
* The checked production paths remain Rust. PySCF is used for independent
  fixture construction and numerical auditing.

The post-submission releases built on that fixed baseline. v0.2.0 added
general active spaces and checked numerical contracts. v0.3.0 added
restartable disk-backed Davidson storage. v0.4.0 added deterministic,
memory-budgeted parallel sigma evaluation. v0.5.0 now adds symmetry-resolved
large exact sectors and audited HPC evidence.

## New all-electron H2O/cc-pVDZ result

The earlier v0.1.1 benchmark intentionally kept point-group symmetry disabled
and established the 1,806,590,016-determinant memory boundary without claiming
a converged FCI energy.

For the new production calculation, the only intentional feasibility change
is exact reduction to the C₂ᵥ A1 block. All other requested conditions remain
unchanged:

* the same `R(O-H)=0.967 Å`, `angle(H-O-H)=107.6°` geometry;
* the same spherical cc-pVDZ basis;
* all ten electrons with no frozen core;
* 24 spatial orbitals;
* the singlet `Nalpha=Nbeta=5`, `MS2=0` sector;
* the same finite-basis Hamiltonian convention;
* residual acceptance at `1e-7`.

The A1 sector contains 451,681,246 determinants. SCNet job `23008083`
completed normally and returned:

| Quantity | Value |
|---|---:|
| Rust Davidson FCI (reported precision) | **`-76.24321859 E_h`** |
| residual norm | **`6.602e-8`** |
| iterations | 21 |
| wall time | 3:55:43 |
| Slurm step MaxRSS | 222.257 GiB (transcribed; raw `sacct` unavailable) |

The production input, SHA-256, unedited Slurm logs, and machine-readable
result are committed. A same-input PySCF 2.14.0 calculation through CCSD(T)
provides a separately executed hierarchy and scale check, not an independent
full-CI oracle. CCSD(T) lies
`0.647144 mE_h` above the Rust FCI result, consistent with the scale of the
cited all-electron water literature without treating its different geometry
as a direct total-energy oracle.

Evidence:

* [corrective final report](https://github.com/JunkaiWang-TheoPhy/quantum-harness-129-workbench-rust/blob/codex/final-competition-submission/reports/final-competition-summary.md)
* [full C₂ᵥ FCI report](https://github.com/JunkaiWang-TheoPhy/quantum-harness-129-workbench-rust/blob/codex/final-competition-submission/reports/h2o-ccpvdz-c2v-fci.md)
* [machine-readable result](https://github.com/JunkaiWang-TheoPhy/quantum-harness-129-workbench-rust/blob/codex/final-competition-submission/fixtures/h2o-ccpvdz-ae/fci-c2v-xh5-result.json)
* [same-input PySCF cross-check](https://github.com/JunkaiWang-TheoPhy/quantum-harness-129-workbench-rust/blob/codex/final-competition-submission/fixtures/h2o-ccpvdz-ae/pyscf-crosscheck.json)
* [scientific data provenance](https://github.com/JunkaiWang-TheoPhy/quantum-harness-129-workbench-rust/blob/codex/final-competition-submission/reports/data-provenance.md)

## SCNet evidence and exact claim boundary

The independent SCNet workflow rebuilt and tested the pinned v0.4.0 source
offline. All 18 robustness cases and all 216 repeated Davidson solves
converged. The maximum absolute energy deviation was `8.10e-13 E_h`, and the
observed peak was 560 allocated CPUs across ten nodes.

A 1,008-CPU gang experiment was submitted with 72 independent solver
processes. The committed evidence does not claim that 1,008 CPUs were
observed, and it does not present task-parallel ensemble throughput as
multi-node strong scaling of one FCI solve.

The release also includes compact point-group sectors throughout the solver,
the 1,002,708-determinant all-electron H2O/DZ result, the
28,233,466-determinant frozen-core H2O/DZP result, stretched-water CC
validation, block Davidson excited roots, and a full-rank H4 UCC check.

* [v0.5.0 release](https://github.com/JunkaiWang-TheoPhy/quantum-harness-129-workbench-rust/releases/tag/v0.5.0)
* [SCNet report](https://github.com/JunkaiWang-TheoPhy/quantum-harness-129-workbench-rust/blob/codex/final-competition-submission/reports/scnet-hpc-benchmark.md)
* [continuous verification](https://github.com/JunkaiWang-TheoPhy/quantum-harness-129-workbench-rust/actions/workflows/ci.yml)

The repository, release, reports, archived logs, machine-readable evidence,
and CI checks are public and ready for review. The corrective report records
the unavailable raw accounting and production-source artifacts explicitly.
